...
figma.on('run', (event) => __awaiter(void 0, void 0, void 0, function* () {
    // Load settings and UI state first, as all commands might need them
    try {
        const savedSettings = yield figma.clientStorage.getAsync('quickSnapSettings');
        if (savedSettings) {
        ...
        const savedUIState = yield figma.clientStorage.getAsync('quickSnapUIState');
        if (savedUIState) {
...
function setupMessageHandlers() {
    figma.ui.onmessage = (msg) => __awaiter(this, void 0, void 0, function* () {
        if (msg.type === 'save-settings') {
            ...
            yield figma.clientStorage.setAsync('quickSnapSettings', JSON.stringify(settings));
            ...
            yield figma.clientStorage.setAsync('quickSnapSettings', JSON.stringify(settings)); // Save settings
            // Update Figma's native export settings for selected nodes
            updateNativeExportSettings();
            yield performExport(); // Perform export after settings are updated
        }
            ...
            yield handleCopyFill(copySettings);
        }
            ...
            yield figma.clientStorage.setAsync('quickSnapUIState', JSON.stringify(uiState));
        }
        else if (msg.type === 'save-ui-state') {
            // Handle UI state changes (expanded sections, etc.)
            uiState = Object.assign(Object.assign({}, uiState), msg.uiState);
            yield figma.clientStorage.setAsync('quickSnapUIState', JSON.stringify(uiState));
        }
        ...
        try {
            ...
            for (const node of selectedNodes) {
                try {
                    ...
                    const imageBytes = yield node.exportAsync(exportOptions);
...
function handleCopyFill(settings) {
    return __awaiter(this, void 0, void 0, function* () {
        const selectedNodes = figma.currentPage.selection;
        if (selectedNodes.length === 0) {
...
function performSnapExport() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield new Promise(resolve => setTimeout(resolve, 100)); // 100ms should be plenty
